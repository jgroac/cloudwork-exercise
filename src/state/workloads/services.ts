...
export class WorkloadService {
  ...
  public create({ complexity }: { complexity: number }) {
    ...
    const timeToFinishInSeconds = complexity;
    const timeToFinishInMilliseconds = timeToFinishInSeconds * 1000;
    const completeDate = moment()
      .add(timeToFinishInMilliseconds, 'milliseconds')
      .toDate();
    const timer = setTimeout(
      () => this.completeWorkload(work),
      timeToFinishInMilliseconds
    );
